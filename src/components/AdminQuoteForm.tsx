...
export function AdminQuoteForm({ initial }: Props) {
  ...
  async function handleDelete() {
    if (!initial) return;
    if (!confirm("Bu sözü silmek istediğine emin misin?")) return;

    setLoading(true);
    const response = await fetch(`/api/quotes/${initial.id}`, { method: "DELETE" });
    if (!response.ok) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      <div>
        <label htmlFor="text" className="mb-1 block text-sm font-medium text-stone-700">
          Söz
        </label>
        <textarea
          id="text"
          ...
          placeholder="Hakkında sayfasının altında görünecek söz…"
        />
      </div>
      ...
      <div>
        ...
        <input
          ...
          placeholder="Örn: Anonim, Rumi, kendi sözün"
        />
      </div>
