...
export default async function WifeWriteChildrenPage() {
  ...
  try {
    const [messages, authorReady] = await Promise.all([
      getFamilyMessagesByAuthorAndAudience("wife", "children"),
    ...
    return (
      <FamilyShell
        role={role}
        introOverride="Çocuklarımız için yazabilirsin. Yazdıkların onların panelinde görünür."
      >
        {!authorReady && (
        ...
        <WifeWriteDashboard messages={messages} audience="children" authorReady={authorReady} />
      </FamilyShell>
    );
  } catch (error) {
