      ...
      <main className="mx-auto min-h-screen max-w-5xl px-4 py-6 sm:px-6 sm:py-10">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          <div>
            ...
            <h1 className="mt-2 font-serif text-2xl text-stone-900 sm:text-3xl">Metinler</h1>
            <p className="mt-2 text-sm text-stone-600">
              Konular, metinler, etiketler ve Hakkında sözlerini buradan yönet.
            </p>
          </div>
        ...
        <div className="mt-6 space-y-3 md:hidden">
          {nodes.length === 0 ? (
            <div className="rounded-xl border border-stone-200 bg-white p-6 text-center text-sm text-stone-500">
              Henüz metin yok. İlk metnini ekle.
            </div>
          ) : (
            nodes.map((node) => (
              <article
              ...
              >
                ...
                <div className="mt-3 flex items-center justify-between text-sm">
                  <span className="truncate text-stone-600">{node.tags || "—"}</span>
                  <Link
              ...
              {nodes.length === 0 ? (
                ...
                nodes.map((node) => (
                  <tr key={node.id} className="border-b border-stone-100 last:border-0">
                    ...
                    <td className="px-4 py-3 font-medium text-stone-900">{node.title}</td>
                    <td className="px-4 py-3 text-stone-600">{node.tags || "—"}</td>
